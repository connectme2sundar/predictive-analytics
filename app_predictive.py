...
all_feature_cols = [c for c in df_features.columns if c not in ['date', 'sales']]

# Drop rows with NaN from feature engineering
model_df = df_features.dropna().reset_index(drop=True)

# After dropping NaN, some feature columns might have been lost if they were all NaN
# So recompute available feature columns from model_df
feature_cols = [c for c in all_feature_cols if c in model_df.columns]
...
# Build future dataframe with same columns as historical
...
for i, row in future_df.iterrows():
    if 'marketing_spend' in df.columns:
        future_df.loc[i, 'marketing_spend'] = last_row['marketing_spend'] * (1.02 ** (i+1)) + np.random.normal(0, 100)
    if 'website_traffic' in df.columns:
        future_df.loc[i, 'website_traffic'] = last_row['website_traffic'] * (1.015 ** (i+1)) + np.random.normal(0, 300)
    if 'competitor_activity' in df.columns:
        future_df.loc[i, 'competitor_activity'] = np.clip(last_row['competitor_activity'] + np.random.normal(0, 1), 0, 10)
    if 'economic_index' in df.columns:
        future_df.loc[i, 'economic_index'] = last_row['economic_index'] + 0.5 * (i+1) + np.random.normal(0, 1)

# Combine historical + future
combined = pd.concat([df[['date', 'year', 'month', 'sales'] + [c for c in ['marketing_spend', 'website_traffic', 'competitor_activity', 'economic_index'] if c in df.columns]], future_df], ignore_index=True)

# Apply feature engineering to combined data
combined_features = create_features(combined)

# Get the actual feature columns that exist in combined_features
combined_feature_cols = [c for c in feature_cols if c in combined_features.columns]

future_start = len(df)
for i in range(future_start, len(combined_features)):
    # Update lag features from already-predicted values
    if i >= 1 and 'sales_lag_1' in combined_feature_cols:
        combined_features.loc[i, 'sales_lag_1'] = combined_features.loc[i-1, 'sales']
    if i >= 2 and 'sales_lag_2' in combined_feature_cols:
        combined_features.loc[i, 'sales_lag_2'] = combined_features.loc[i-2, 'sales']
    if i >= 3 and 'sales_lag_3' in combined_feature_cols:
        combined_features.loc[i, 'sales_lag_3'] = combined_features.loc[i-3, 'sales']
    if i >= 1 and i >= 3 and 'sales_roll_mean_3' in combined_feature_cols:
        combined_features.loc[i, 'sales_roll_mean_3'] = combined_features.loc[i-3:i-1, 'sales'].mean()

    # Predict using only columns that exist
    X_f = combined_features.loc[i, combined_feature_cols].values.reshape(1, -1)
    pred = best_model.predict(X_f)[0]
    combined_features.loc[i, 'sales'] = pred

forecast_df = combined_features.iloc[future_start:].copy()
